...
class info:
        def __init__(self,addagent,addmap,numAgent):
            self.fagents=open(addagent,'r')
        ...
        def read_agent(self):
            ...
            """store the constraint, in linux we store the barriers as well"""
        ...
        def displayAIDetail(self, canvas, inputtxt, textbox):
                ...
                for tt in range(len(tmpArrayList[0])):
                    ...
                    temp+="\n"
                    textbox.insert("end",temp,'normal')
                ...
                textbox.insert("end",temp,'constraint')
        ...
        def linemaker(self,screen_points):
            ...
            for (y,x) in screen_points:
                # If its the first point in size set, set x0,y0 to the values
            ...
            if doBackward:
                ...
                if (t in self.cons_time_dict.keys()):
                    for value in self.cons_time_dict[t]:
                        ...
                        #add text description to the frame
                        frame.text.insert("end", "Constraint :\n"+"agent "+str(value[2])+" ("+str(value[0][0]-1)+","+str(value[0][1]-1)+")"+"\n",'constraint')
                        frame.text.see("end")
                        if (value[1][0],value[1][1]) != (0,-1):
                            canvas.itemconfig(self.CanvasMap[value[1][0]][value[1][1]], fill='#0000FF')
                            frame.text.insert("end", " ("+str(value[1][0]-1)+","+str(value[1][1]-1)+")\n" ,'constraint')
                for i in range(len(self.AgentsPos)):
                    #if still moving.
