import sys
# from lazycbs import init  """linux features"""
...
class MyCanvas(Canvas):
    def __init__(self, parent, **kwargs):
        ...
        """Allow the user to drag the scene"""
        # self.canvas = canvas
...
class rightFrame(Frame):
    def __init__(self, master):
        ...
        self.playButton = Button(self, text='Pause', width=10, height=3, bg='red', fg='black',
                                 command=self.playVisualizer)
        ...
        self.speedUpButton = Button(self, text='<<<', width=5, height=3, bg='white', fg='black',
                                    command=lambda: self.speedChange(0.05))
        self.speedUpButton.grid(row=5, column=0, pady=10)
        self.speedDownButton = Button(self, text='>>>', width=5, height=3, bg='white', fg='black',
                                      command=lambda: self.speedChange(-0.05))
...
class topFrame(Frame):
    def __init__(self, master, canvas):
        ...
        """setup text box, button in topFrame"""
    ...
    def agentDetailWindow(self):
        if (self.agentDetailWindow == None):
            ...
            self.exitButton = Button(self.newframe, text="Exit", highlightbackground="#56B426",
                                     command=self.agentDetailDestroy)
    ...
    def askQuestionWindow(self):
        if (self.askQuestionWindow == None):
            ...
            self.printButton2 = Button(self.newframe2, text="run!",
                                       command=lambda: self.runModel(self.agent, self.location1, self.location2,
                                                                     self.time, self.cost, self.t2))
            self.printButton2.pack()
            self.exitButton2 = Button(self.newframe2, text="Exit", highlightbackground="#56B426",
                                      command=self.askQuestionWindowDestroy)
        ...
        if boolean:
            with open("test1.scen", 'w') as out:
                ...
                for i in range(len(Info.AgentsPos)):
                    tmp = str(i) + '\t' + 'debug-6-6.map' + '\t' + str(len(Info.BinaryMap[0])) + '\t' + str(
                        len(Info.BinaryMap)) + '\t' + str(
                        Info.AgentsPos[i][min(Info.currentTime, len(Info.AgentsPos[i]) - 1)][1] - 1) + '\t' + str(
                        Info.AgentsPos[i][min(Info.currentTime, len(Info.AgentsPos[i]) - 1)][0] - 1) + '\t' + str(
                        Info.AgentsPos[i][-1][1] - 1) + '\t' + str(Info.AgentsPos[i][-1][0] - 1) + '\t' + str(8) + '\n'
                    out.write(tmp)
...
if __name__ == "__main__":
    # run in windows: python run.py test_2.txt debug-6-6.map.ecbs 2
    # run in linux: python3 run.py test_2.txt debug-6-6.map.ecbs 2
    try:
        addAgent = sys.argv[1]
        addMap = sys.argv[2]
    ...
    """linux feature, generate soluiton from original locations"""
    """agent structure: Agent 0: (0,4) (0,3)"""
    """map structure: 3,3
                      1,1,1
                      1,0,1
                      1,1,1"""
    ...
    # Construct size simple root window
    root = Tk()
    ...
    the_canvas = MyCanvas(root, width=1200, height=720, bg="#d1d1d1")
